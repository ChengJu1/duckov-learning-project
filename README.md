...
阶段 0 和阶段 1 已完成。阶段 2 正在进行，当前已加入鼠标瞄准方向，尚未加入射击、敌人和伤害。
...
窗口中使用 `WASD` 或方向键移动黄色玩家方块；玩家身上的浅黄色短线会朝向鼠标。接触绿色物品后，物品消失且携带数量增加；随后进入左侧蓝色区域完成撤离。物品会结算到 `Stash items`，按 `R` 开始下一局。空手不能撤离，也不能在局中重开。
...
1. 人工验收鼠标瞄准方向。
2. 左键沿瞄准方向生成一个基础弹丸，并在离开地图后清理。
3. 再加入一个静态敌人和最小生命值、伤害规则。
