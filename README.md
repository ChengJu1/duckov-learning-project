...
阶段 0 和阶段 1 已完成。阶段 2 正在进行，鼠标瞄准、基础弹丸、静态敌人、生命值和弹丸伤害均已通过人工验收。敌人追踪移动已实现，等待人工验收；暂不攻击玩家。
...
红色敌人从右上方出生，以 90 像素/秒追踪玩家（玩家为 240 像素/秒），中心距离约 56 像素时停止；玩家走远后继续追踪。初始生命值为 100，每颗命中的弹丸造成 25 伤害并消失。命中四次后敌人消失并停止移动，左上角显示 `Enemy defeated`；新局恢复出生位置和满血。敌人暂不攻击、不会阻挡玩家移动，击败敌人不是撤离前提。
...
1. 人工验收敌人追踪、接近后停止、击败/撤离冻结和新局恢复。
2. 加入玩家生命值、敌人攻击和失败结算规则。
