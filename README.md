...
阶段 0 和阶段 1 已完成。阶段 2 正在进行，鼠标瞄准和基础弹丸已通过人工验收；静态敌人、生命值和弹丸伤害已实现，等待人工验收。敌人暂不移动或反击。
...
右上方红色方块是静态敌人，初始生命值为 100，每颗命中的弹丸造成 25 伤害并消失。命中四次后敌人消失，左上角显示 `Enemy defeated`；新局恢复满血。敌人不会阻挡玩家移动，击败敌人不是撤离前提。
...
1. 人工验收静态敌人的命中扣血、击败和新局恢复。
2. 实现简单敌人移动。
3. 加入玩家生命值、敌人攻击和失败结算规则。
